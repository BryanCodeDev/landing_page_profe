...
const ExerciseCard = ({ exercise }) => {
  ...
  return (
    <div className={`
      ...
      transition-all duration-250 p-5 mb-3
    `}>
      <div className="flex flex-col sm:flex-row items-start gap-4">

        {/* Ícono tipo archivo */}
        <div className={`shrink-0 w-11 h-11 rounded-xl bg-${accent}-50 border border-${accent}-100 flex items-center justify-center`}>
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex flex-wrap items-center gap-2 mb-1.5">
            ...
            <span className={`inline-flex items-center gap-1.5 text-xs font-semibold px-2.5 py-0.5 rounded-full ${level.cls}`}>
          ...
          <div className="flex flex-wrap gap-1.5 mb-3">
            {exercise.skills.map(skill => (
              <span key={skill}
                className="text-xs font-medium bg-slate-50 border border-slate-200 text-slate-600 px-2 py-0.5 rounded-md hover:bg-slate-100 transition-colors">
                {skill}
              </span>
            ))}
          </div>
        ...
        <div className="shrink-0 self-start sm:self-center">
          <a
            ...
            className={`
              flex items-center gap-2 text-xs font-bold px-4 py-2.5 rounded-xl
              bg-${accent}-600 text-white
              hover:bg-${accent}-700 hover:shadow-md hover:shadow-${accent}-100
